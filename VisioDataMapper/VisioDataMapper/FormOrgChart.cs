...
{
    public class FormOrgChart : Form
    {
        ...
        private const string StructurePrompt = "上面是我的功能模块资料，根据资料里的内容，帮我画一个功能模块图，用mermaid中的mindmap格式输出，要求内容为中文";
        ...
        private void InitializeComponent()
        {
            ...
            Label lblTutorialTip = new Label { Text = "请将结构内容粘贴到下方", Location = new Point(15, 95), AutoSize = true };

            btnPasteOptimize = new Button { Text = "粘贴并优化格式", Location = new Point(15, 120), Size = new Size(140, 28) };
            btnPasteOptimize.Click += btnPasteOptimize_Click;

            lblTitle = new Label { Text = "图标题:", Location = new Point(180, 125), Size = new Size(55, 20) };
            txtTitle = new TextBox { Text = "公司组织架构图", Location = new Point(240, 121), Size = new Size(360, 25) };

            txtStructure = new RichTextBox { Location = new Point(15, 160), Size = new Size(1040, 360), ScrollBars = RichTextBoxScrollBars.Both, WordWrap = false, Font = new Font("Microsoft YaHei", 11F, FontStyle.Regular) };
            ...
            this.Resize += FormOrgChart_Resize;
            this.Shown += FormOrgChart_Shown;
            LayoutControls();
        }

        private void LoadSampleData()
        {
            txtStructure.Text = @"总经理
    副总经理（销售）
        销售一部
        销售二部
        市场部
    ...
    财务总监
        ...
        private void FormOrgChart_Resize(object sender, EventArgs e)
        {
            LayoutControls();
        }

        private void FormOrgChart_Shown(object sender, EventArgs e)
        {
        ...
        private void btnPasteOptimize_Click(object sender, EventArgs e)
        {
            ...
            {
                txtStructure.Text = OptimizeStructureText(Clipboard.GetText());
                AppendLog("成功从剪贴板粘贴文本并优化格式。");
            }
            else
            {
        ...
        private string OptimizeStructureText(string text)
        {
            ...
            foreach (string rawLine in lines)
            {
                ...
                string expandedLine = rawLine.Replace("\t", "    ").TrimEnd();
                int indent = CountLeadingSpaces(expandedLine);
                string content = expandedLine.TrimStart();
                int outlineLevel = TryGetOutlineLevel(content);

                if (outlineLevel > 0 && indent == 0)
                {
                    indent = (outlineLevel - 1) * 4;
                }

                content = Regex.Replace(content, @"^\d+(?:\.\d+)+[\.\)、)]?\s*", string.Empty);
                content = Regex.Replace(content, @"^([\-*•●○▪▫]\s*)+", string.Empty);
                content = Regex.Replace(content, @"^((\d+|[A-Za-z]|[一二三四五六七八九十]+)[\.\)、)]\s*)+", string.Empty);
                content = content.Trim();

                if (content.Length > 0)
                {
                    optimizedLines.Add(new string(' ', indent) + content);
                }
            }
        ...
        private int TryGetOutlineLevel(string content)
        {
            Match match = Regex.Match(content, @"^(\d+(?:\.\d+)+)[\.\)、)]?\s+");
            if (!match.Success)
            {
                return 0;
            }
        ...
        private void btnGenerate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (root == null)
                {
                    MessageBox.Show("无法解析结构内容，请确保文本非空并且具有层级关系！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                AppendLog("生成绘图完成！");
                MessageBox.Show("架构图生成成功！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
        ...
        private TreeNode ParseTree(string text)
        {
            ...
            TreeNode root = null;
            var lastNodes = new Dictionary<int, TreeNode>();
            ...
            foreach (var line in lines)
            {
                ...
                TreeNode node = new TreeNode { Text = content };

                if (root == null)
                {
                    root = node;
                    lastNodes[indent] = node;
                }
                else
                {
                    // Find parent: the last node seen at a smaller indent
                    int parentIndent = -1;
                    foreach (var key in lastNodes.Keys)
                    {
                        if (key < indent && key > parentIndent)
                        {
                            parentIndent = key;
                        }
                    }
                    ...
                    if (parentIndent != -1)
                    {
                        TreeNode parent = lastNodes[parentIndent];
                        node.Parent = parent;
